...
export async function fetchLivePrices(apiKey, brand, name, catKey, retailers, options = {}) {
  ...
  const candidates = await Promise.all(retailers.map(async retailer => {
    ...
    console.log('[trailhead-price-search]', JSON.stringify({ brand, name, retailer: retailer.domain,
      pages: pages.map(p => ({ title: p.title || null, url: p.url })), providerError: firstError }));
  ...
  console.log('[trailhead-price-fetch]', JSON.stringify({ requested: urls,
    results: [...byUrl.values()].map(p => ({ url: p.url, final_url: p.final_url || null, title: p.title || null, text_length: typeof p.text === 'string' ? p.text.length : null })),
  ...
  const results = candidates.map(c => {
    ...
    for (const candidate of c.pages) {
      ...
      console.log('[trailhead-price-page]', JSON.stringify({ retailer: c.retailer.domain,
        url: candidate.url, title: page.title || null,
        headings: [...page.text.matchAll(/^#{1,6}\s+(.+)$/gm)].slice(0, 12).map(m => m[1]),
        section: section?.text?.slice(0, 1800) || null,
        price_lines: page.text.split(/\n/).filter(line => /(?:NZ\$|NZD|\$)\s*\d|\bprice\b/i.test(line)).slice(0, 20) }));
      if (!section) continue;
