...
export async function fetchLivePrices(apiKey, brand, name, catKey, retailers, options = {}) {
  ...
  const candidates = await Promise.all(retailers.map(async retailer => {
    ...
    let result = await runSearch(`${brand} ${name} site:${retailer.domain}`);
    if (result.error) return { retailer, error: result.error, pages: [] };
    let pages = relevant(result.data);
    // Search engines occasionally ignore a site filter for a retailer. Retry
    // once with the retailer name, then apply the same strict filters.
    if (!pages.length && Date.now() < deadline) {
      const retry = await runSearch(`${brand} ${name} ${retailer.name} NZ`);
      if (!retry.error) pages = relevant(retry.data);
    }
