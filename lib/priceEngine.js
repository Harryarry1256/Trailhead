...
function productSection(text, brand, name) {
  ...
  const heading = headings.find(h => titleMatches(h[1], brand, name));
  ...
  let section = text.slice(start, nextHeading >= 0 ? start + nextHeading : start + 8000);
  // A title-only heading followed by an arbitrary body price is too easy to
  // confuse with a related-item price. Real product pages expose a price
  // heading or an explicit price label in the anchored section.
  if (!/^##\s+.*(?:\$|\d[\d,.]*\.\d{2})/m.test(section) && !/\b(?:price|now|sale)\s*:/i.test(section)) return null;
...
export async function fetchLivePrices(apiKey, brand, name, catKey, retailers, options = {}) {
  ...
  const candidates = await Promise.all(retailers.map(async retailer => {
    ...
    const pages = result.data.results.filter(r => r && isProductUrl(r.url, retailer.domain) && titleMatches(r.title, brand, name));
  ...
  const results = candidates.map(c => {
    ...
    for (const candidate of c.pages) {
      ...
      if (!isProductUrl(finalUrl, c.retailer.domain) || !titleMatches(page.title, brand, name)) continue;
