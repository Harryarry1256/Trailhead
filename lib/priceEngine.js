...
export async function fetchLivePrices(apiKey, brand, name, catKey, retailers, options = {}) {
  ...
  const results = candidates.map(c => {
    ...
    for (const candidate of c.pages) {
      ...
      console.log('[trailhead-html]', JSON.stringify({ retailer: c.retailer.domain,
        url: candidate.url, type: typeof page.text, length: page.text.length,
        preview: page.text.slice(0, 1200),
        price_context: (page.text.match(/.{0,120}(?:NZ\$|NZD|\$|price|offers|amount).{0,180}/ig) || []).slice(0, 20) }));
