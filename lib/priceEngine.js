...
import { retryDelay } from './provider.js';
...
async function requestJson(fetchImpl, url, init, deadline) {
  ...
  try {
    ...
    if (!response.ok) return { error: response.status === 429 ? 'rate_limited' : `provider_http_${response.status}`,
      retryAfter: response.status === 429 ? Math.ceil(retryDelay(response.headers?.get('Retry-After')) / 1000) : undefined };
...
export async function fetchLivePrices(apiKey, brand, name, catKey, retailers, options = {}) {
  ...
  const providerFetch = options.providerFetch || fetchImpl;
  const deadline = Date.now() + (options.timeoutMs ?? 24000);
  const headers = { 'X-API-Key': apiKey };
  const candidates = await Promise.all(retailers.map(async retailer => {
    // Discovery is separate from prices: reuse known URLs, but fetch and
    // validate their product identity and current price on every refresh.
    const known = (options.knownListings || []).filter(r => r.retailer === retailer.name &&
      r.status === 'verified' && isProductUrl(r.url, retailer.domain) &&
      titleMatches(r.product_title, brand, name));
    if (known.length) return { retailer, pages: known.slice(0, 2).map(r => ({
      url: r.url, title: r.product_title
    })), reused: true };

    const runSearch = query => requestJson(providerFetch,
    ...
    for (const query of queries) {
      ...
      // Changing the query cannot fix a rejected request. Retrying it only
      // consumes more allowance and extends the account-wide rate limit.
      if (result.error) { firstError = result.error; retryAfter = result.retryAfter; break; }
      pages = relevant(result.data);
      if (pages.length) break;
    }
    if (!pages.length && firstError) return { retailer, error: firstError, retryAfter, pages: [] };
  ...
  if (urls.length) {
    fetched = await requestJson(providerFetch, 'https://api.fetch.tinyfish.ai', {
  ...
  const results = await Promise.all(candidates.map(async c => {
    ...
    if (c.error) return { ...base, price: 'Lookup unavailable', status: 'unavailable', error_code: c.error, retry_after: c.retryAfter };
    ...
    for (const candidate of c.pages) {
      ...
      if (!page || typeof page.text !== 'string') {
        failure = { ...failure, price: 'Lookup unavailable', status: 'unavailable',
          error_code: fetched.error || errors.get(candidate.url) || 'missing_page', retry_after: fetched.retryAfter };
        continue;
      }
  ...
  // A removed/renamed known page must be rediscovered on a later run.
  // Never replace a saved price with that failed page's content.
  return {
    rediscover: candidates.some((c, i) => c.reused && results[i].status !== 'verified'),
    results,
    retryAfter: Math.max(0, ...results.map(r => r.retry_after || 0)),
