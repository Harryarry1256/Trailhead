...
export async function fetchLivePrices(apiKey, brand, name, catKey, retailers, options = {}) {
  ...
  const results = candidates.map(c => {
    ...
    for (const candidate of c.pages) {
      ...
      console.log('[trailhead-price-result]', JSON.stringify({ retailer: c.retailer.domain,
        url: candidate.url, page_title: page.title || null,
        headings: [...page.text.matchAll(/^#{1,6}\s+(.+)$/gm)].slice(0, 10).map(m => m[1]),
        preview: page.text.slice(0, 700), section: section?.text?.slice(0, 900) || null,
        price_lines: page.text.split(/\n/).filter(line => /(?:NZ\$|NZD|\$)\s*\d|\b\d[\d,.]*\.\d{2}\b|\bprice\b/i.test(line)).slice(0, 30),
        price: section ? extractPrice(section.text, CATEGORY_BOUNDS[catKey]) : null }));
      if (!section) continue;
