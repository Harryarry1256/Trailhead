...
// Buy-now-pay-later installment amounts ("or 4 payments of $119.75 with
// Afterpay") are always specifically preceded by "payments of" — this
// only rejects a match that has that phrase directly before it, so the
// REAL price (which usually comes right before that whole sentence) is
// left alone rather than also getting rejected for being nearby.
const NOISE_BEFORE_INSTALLMENT = /\d+\s*(?:weekly|fortnightly|monthly)?\s*payments?\s*of\s*$/i;
...
// A much stricter check for page/result TITLES specifically (not full
// body text). Some brand and model words are also ordinary English
// words ("Trek" = hiking, "Marlin" = the fish), so scanning a whole
// page's body for those words appearing *anywhere* is fooled easily by
// a totally unrelated page. A genuine product page's actual <title> will
// contain the model name as one contiguous phrase; an unrelated page's
// title essentially never will by coincidence.
function normalize(s) {
  return s.toLowerCase().replace(/[^a-z0-9]+/g, '');
}
function titleMatches(title, brand, name) {
  if (!title) return false;
  const normTitle = normalize(title);
  return normTitle.includes(normalize(brand)) && normTitle.includes(normalize(name));
}
...
function extractPrice(text, bounds, preferEarliest, anchorText) {
  ...
  while ((match = re.exec(scoped)) !== null) {
    ...
    // Wider window, but ONLY checking before the match (not after) —
    // specifically catches "4 payments of $X", not the real price that
    // happens to be followed by that phrase elsewhere in the sentence.
    const beforeWide = scoped.slice(Math.max(0, match.index - 40), match.index);
    if (NOISE_BEFORE_INSTALLMENT.test(beforeWide)) continue;
...
async function findCandidate(apiKey, retailer, brand, name) {
  ...
  const relevant = onDomain.filter(r => titleMatches(r.title, brand, name));
...
async function fetchAndExtract(apiKey, candidates, bounds, brand, name) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        for (const r of (data.results || [])) {
          if (r.url && r.text) pageByUrl[r.url] = { text: r.text, title: r.title || '' };
        }
      }
    } catch {
  ...
  for (const c of withUrls) {
    const page = pageByUrl[c.candidate.url];
    const pageText = page ? page.text : null;
    // Prefer the fetched page's actual <title> (much stronger signal —
    // see titleMatches above); fall back to the search result's title if
    // Fetch didn't return one for some reason.
    const pageIsRelevant = pageText && titleMatches(page.title || c.candidate.title, brand, name);
