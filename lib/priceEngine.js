...
// Buy-now-pay-later services (Afterpay, Zip, Laybuy, Genoapay, humm,
// Klarna) show an installment amount right next to the real price —
// e.g. "or 4 payments of $119.75 with Afterpay". That fraction is not
// the actual product price and must be rejected outright, not just
// deprioritized. Checked both before AND after the match, since the
// installment phrasing can appear on either side depending on the site.
const NOISE_NEARBY = /(afterpay|zip ?pay|laybuy|genoapay|humm|klarna|openpay|\d+\s*(?:weekly|fortnightly|monthly)?\s*payments?\s*of|interest[\s-]?free|instal?ments?)/i;
...
function extractPrice(text, bounds, preferEarliest, anchorText) {
  ...
  while ((match = re.exec(scoped)) !== null) {
    ...
    // Check a wider window on both sides for BNPL/installment phrasing.
    const nearby = scoped.slice(Math.max(0, match.index - 60), match.index + 60);
    if (NOISE_NEARBY.test(nearby)) continue;
