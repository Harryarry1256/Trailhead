  ...
  const results = await Promise.all(candidates.map(async c => {
    ...
    for (const candidate of c.pages) {
      ...
      if (price === null) {
        ...
        console.log('[trailhead-direct]', JSON.stringify({ retailer: c.retailer.domain,
          url: finalUrl, status: direct?.status || null, length: direct?.text?.length || 0,
          has_title: Boolean(direct?.text && titleMatches(direct.text, brand, name)),
          price_context: direct?.text ? (direct.text.match(/.{0,80}(?:NZ\$|NZD|\$|price).{0,120}/ig) || []).slice(0, 12) : [],
          raw_context: direct?.raw ? (direct.raw.match(/.{0,140}(?:579884|api|graphql|price|product).{0,220}/ig) || []).slice(0, 30) : [] }));
