...
async function fetchDirectProductText(fetchImpl, url, deadline) {
  ...
  try {
    ...
    return htmlToProductText(await response.text());
  } catch { return null; }
}
...
export async function fetchLivePrices(apiKey, brand, name, catKey, retailers, options = {}) {
  ...
  const results = await Promise.all(candidates.map(async c => {
    ...
    for (const candidate of c.pages) {
      ...
      if (price === null) {
        const directText = await fetchDirectProductText(fetchImpl, finalUrl, deadline);
        const directSection = directText && productSection(directText, brand, name);
