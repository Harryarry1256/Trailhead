      ...
      if (search && response.ok) {
        try {
          ...
          if (Array.isArray(data.results)) {
            // No-result discovery is retried after six hours, not on every
            // catalog pass. Failures are never cached as empty results.
            const ttl = data.results.length ? 3600 : 6 * 3600;
